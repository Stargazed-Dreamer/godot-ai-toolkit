...
func _tool_update_node_property(params: Dictionary) -> Dictionary:
	...
	# [gdflow] 读回验证：类型转换失败/物理层拒绝赋值时不再静默返回 success
	var converted_str: String = str(converted_value)
	var actual_str: String = str(new_value)
	if converted_str != actual_str and property_value != null:
		var converted_type: int = typeof(converted_value)
		if converted_type in [TYPE_VECTOR2, TYPE_VECTOR2I, TYPE_VECTOR3, TYPE_VECTOR3I, TYPE_COLOR, TYPE_RECT2, TYPE_PACKED_VECTOR2_ARRAY, TYPE_PACKED_COLOR_ARRAY]:
			return {
				"status": "verify_failed",
				"error": "Property set did not take effect (type conversion or assignment failed): expected %s, got %s" % [converted_str, actual_str],
				"node_path": node_path,
				"property_name": property_name,
				"expected": converted_str,
				"actual": actual_str
			}
...
func _convert_value_for_property(node: Node, property_name: String, value: Variant) -> Variant:
	...
	# [gdflow] 数组输入支持：JSON 数组此前会落到兜底原样返回，导致 set 静默失败
	if value is Array:
		match property_type:
			TYPE_VECTOR2, TYPE_VECTOR2I, TYPE_VECTOR3, TYPE_VECTOR3I, TYPE_COLOR, TYPE_RECT2:
				var nums: Array = []
				var all_num: bool = value.size() > 0
				for v in value:
			...
			TYPE_PACKED_VECTOR2_ARRAY, TYPE_PACKED_COLOR_ARRAY, TYPE_PACKED_FLOAT32_ARRAY, TYPE_PACKED_INT32_ARRAY, TYPE_PACKED_STRING_ARRAY:
				return _convert_packed_array(value, property_type)

	return value

## [gdflow] Packed* 数组转换：接受 Vector2/Color 对象数组、{"x":..} 字典数组、扁平数字数组、"Vector2(x, y)" 字符串数组
func _convert_packed_array(value: Array, packed_type: int) -> Variant:
	match packed_type:
		TYPE_PACKED_VECTOR2_ARRAY:
			...
			for v in value:
				if not (v is float or v is int):
					all_num = false
					break
			if all_num and value.size() % 2 == 0:
				var i := 0
				while i < value.size():
					out.append(Vector2(float(value[i]), float(value[i + 1])))
					i += 2
				return out
			for v in value:
				if v is Vector2:
					...
					out.append(Vector2(float(v.get("x", 0.0)), float(v.get("y", 0.0))))
				elif v is String:
					var parts: PackedStringArray = _strip_constructor_prefix(v).replace("(", "").replace(")", "").replace(" ", "").split(",")
					if parts.size() >= 2:
		...
		TYPE_PACKED_COLOR_ARRAY:
			...
			for v in value:
				if v is Color:
					...
					out.append(Color(float(v.get("r", 0.0)), float(v.get("g", 0.0)), float(v.get("b", 0.0)), float(v.get("a", 1.0))))
			return out
